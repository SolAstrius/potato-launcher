...
fn main() {
    ...
    if let Err(err) = inner(dir) {
        let lib = dir.join("src").join("generated.rs");
        let err = format!("{}", err);
...
fn inner(dir: &Path) -> Result<(), Box<dyn Error>> {
    ...
    let data = toml::from_str::<toml::Value>(data)?;
...
fn write_generated(path: &Path, content: &str) -> Result<(), Box<dyn Error>> {
    ...
    if !status.success() {
        return Err(format!("rustfmt failed for {}", path.display()).into());
    }
    ...
    pub fn write(&self, locales: &Locales, indentation: usize, content: &mut String) {
        match &self.val {
            LocaleNodeVal::Parent {
                ...
                if !static_translation_children.is_empty() {
                    let has_short = static_translation_children.iter().any(|(_, index)| {
                        let LocaleNodeVal::Leaf {
                            short_translations, ..
                        } = &locales.nodes[*index].val
                        else {
                            unreachable!();
                        };
            ...
            LocaleNodeVal::Leaf {
                ...
                content.push_str(&INDENTATION[..indentation * 4 + 4]);
                content
                    .push_str("match crate::LANG.load(std::sync::atomic::Ordering::Relaxed) {\n");
...
impl Locales {
    ...
    fn create(&mut self, data: toml::Value) -> Result<Vec<LocaleNodeVal>, Box<dyn Error>> {
        match data {
            toml::Value::Table(map) => {
                ...
                if has_table {
                    ...
                    for (key, value) in map {
                        ...
                        let name = if key == "mod" {
                            "mod_".to_string()
                        } else if !key.starts_with(char::is_alphabetic) {
                            format!("_{key}")
                        } else {
                            key
                        };
                        ...
                        for val in vals {
                            ...
                            {
                                static_translation_children
                                    .push((original_name.clone(), self.nodes.len()));
                            }
            ...
            _ => Err(format!("Unexpected value: {data:?}").into()),
        }
    }
}
